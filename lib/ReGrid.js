...
var ReGrid = function () {
  function ReGrid(wrapper, options) {
  ...
  _createClass(ReGrid, [{
    ...
    value: function renderHeader() {
      // fixed header
      this.header.innerHTML = (0, _utils.getHeaderHTML)(this.data.columns);
    }
  }, {
    key: 'renderBody',
    ...
    value: function setDimensions() {
      ...
      $$('.data-table-col', this.header).forEach(function ($col) {
        var width = $('.content', $col).offsetWidth;

        var _getCellAttr = _this2.getCellAttr($col),
            colIndex = _getCellAttr.colIndex;

        if (!_this2.minWidthMap[colIndex]) {
          // only set this once
          _this2.minWidthMap[colIndex] = width;
        }
      });

      // set initial width as naturally calculated by table's first row
      $$('.data-table-row[data-row-index="0"] .data-table-col', this.bodyScrollable).forEach(function ($cell) {
        var width = $('.content', $cell).offsetWidth;
        var height = $('.content', $cell).offsetHeight;

        var _getCellAttr2 = _this2.getCellAttr($cell),
      ...
      var defaultHeight = $('.data-table-col[data-col-index="0"][data-row-index="0"]', this.bodyScrollable).offsetHeight;
    ...
    value: function bindEditCell() {
      ...
      $(document.body).on('keypress', function (e) {
        ...
        if (e.which === 13 && _this3.$focusedCell && !_this3.$editingCell) {
          _this3.log('editingCell');
          _this3.activateEditing(_this3.$focusedCell);
      ...
      $(document.body).on('keypress', function (e) {
        ...
        if (e.which === 13 && _this3.$editingCell) {
          _this3.log('submitCell');
          _this3.submitEditing(_this3.$editingCell);
          e.stopImmediatePropagation();
        }
      });
    ...
    value: function setColumnWidths() {
      ...
      this.data.columns.map(function (col) {
        ...
        if (_this5.addSerialNoColumn && col.colIndex === 0) {
          return;
        }
    ...
    value: function setStyle(rule, styleMap) {
      var styles = this.$style.innerHTML;
    ...
    value: function getCellAttr($cell) {
      var dataAttributes = Array.from($cell.attributes).map(function (attrNode) {
        return attrNode.name;
      }).filter(function (attr) {
        return attr.startsWith('data-');
      });
      var values = dataAttributes.map(function (attr) {
        return $cell.getAttribute(attr);
      });
      var out = {};

      dataAttributes.map(function (attr, i) {
        attr = attr.replace('data-', '');
        var key = (0, _utils.dashToCamelCase)(attr);
        var value = values[i];
